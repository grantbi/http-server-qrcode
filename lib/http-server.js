...
exports.createServer = function (options, server) {
  return new HttpServer(options, server);
};
 ...
 */
function HttpServer(options, server) {
  options = options || {};
  ...
  before.push(function(req, res, next) {
    let end = res.end;
    ec(req, Object.assign(res, { end:async (data) => {
      let $ = cheerio.load(data);
      await new Promise(async (good, bad) => {
        let ps = []
        let b = $('a').each(async (index, item) => {
          let code = QRCode.toDataURL(`${server.protocol}${server.canonicalHost}:${server.port}/${encodeURIComponent($(item).text())}`);
          ps.push(code);
          let qr = `<div class='qr' style='background-image:url(${await code})'></div>`
          $(item).append(qr)
        })
        await Promise.all(ps).then(good).catch(bad)
      })
      let style = `
        <style>
          body {
